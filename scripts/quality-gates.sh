...
REPO_MODULE="unified_api_contracts"
REPO_ARCH_TIER="0"
...
# Check 13: File size limit (max 900 lines)
...
PRIV_KEY=$(rg "BEGIN RSA PRIVATE KEY|BEGIN PRIVATE KEY|BEGIN EC PRIVATE KEY" \
    --type py --type sh --type yaml --glob "!tests/**" --glob "!scripts/**" . 2>/dev/null || true)
if [ -n "$PRIV_KEY" ]; then
...
# Check 25: pip-audit (blocking)
echo -n "Checking dependency vulnerabilities (pip-audit)... "
if command -v pip-audit &>/dev/null; then
    pip-audit 2>/dev/null && echo -e "${GREEN}PASS${NC}" || { log_fail "pip-audit found vulnerabilities"; CODEX_VIOLATIONS=$((CODEX_VIOLATIONS + 1)); }
elif $PYTHON_CMD -c "import pip_audit" 2>/dev/null; then
    $PYTHON_CMD -m pip_audit 2>/dev/null && echo -e "${GREEN}PASS${NC}" || { log_fail "pip-audit found vulnerabilities"; CODEX_VIOLATIONS=$((CODEX_VIOLATIONS + 1)); }
else
    log_fail "pip-audit required: uv pip install pip-audit"
    CODEX_VIOLATIONS=$((CODEX_VIOLATIONS + 1))
fi

# Check 26: bandit (blocking)
echo -n "Checking security (bandit)... "
if command -v bandit &>/dev/null; then
    run_timeout 30 bandit -r "${REPO_MODULE}/" -ll 2>/dev/null && echo -e "${GREEN}PASS${NC}" || { log_fail "bandit found issues"; CODEX_VIOLATIONS=$((CODEX_VIOLATIONS + 1)); }
else
