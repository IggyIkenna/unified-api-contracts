...
**Current state (batch):** Barchart manual CSV dumps → `BarchartOhlcv15m` schema → market-tick-data-handler → GCS. Instrument: `CBOE:INDEX:VIX-USD`.
...
- **Path**: `market-tick-data-handler/data/vix/vix_intraday-15min_historical-data-*.csv`
- **No API, no live** — migrate when Databento index or IBKR VIX streaming is chosen

## Migration Path

1. **Short term**: Keep Barchart batch; document schema in api-contracts (done)
2. **When Databento index ships**: Add Databento VIX schemas; migrate market-tick-data-handler
