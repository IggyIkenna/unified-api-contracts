...
For each provider we integrate: every endpoint, every message type, every schema the provider exposes must have a corresponding raw schema in `unified_api_contracts_external/<provider>/`. If a provider offers liquidations, we must have the raw schema. If they offer options chain, we must have it. **Gap = audit fail.** Action: add missing raw schemas.

### 4.2 Is there any raw schema not planned to be normalized?

Every class in `unified_api_contracts_external/` must have a normalization path to a canonical type (or a documented reason: e.g. request-only schemas that don't flow to engine). **Orphan = audit fail.** Action: add normalizer or document exception.
...
3. **Add normalizers for gaps:** Create `normalize_<provider>_<type>` for each orphaned schema in §2. Follow existing patterns. **Proxy schemas:** Where provider lacks explicit support (e.g. no ping), use proxy (heartbeat = ping). **Optional fields:** Populate only fields the provider offers; document per-provider which fields are None. in `unified_normalised_contracts/normalize/` (trades.py, orderbooks.py, orders_fills.py, tickers.py). Add: **liquidations.py**, **derivative_tickers.py**, **options_chain.py**, **errors.py**, **fees.py**, **reference_data.py**.
