...
- **`unified_api_contracts/`** — Root package.
  - **`unified_api_contracts_external/`** — Raw request/response/error schemas per venue or external system (Binance, Databento, Tardis, CCXT, etc.). Per-venue dirs contain `schemas.py`, `examples/`, `mocks/` (VCR cassettes; recording done in interfaces). **prime_broker**, **fix**, **nautilus** belong here per rule (currently at root).
  - **`unified_normalised_contracts/`** — Canonical domain, execution, and error types (self-contained; no internal imports).
...
- **Normalised**: Canonical types used by UMI/UOI and services. See `unified_api_contracts.unified_normalised_contracts` and [README Structure](../README.md#structure), [docs/INDEX.md](INDEX.md), [docs/CROSS_VENUE_MATRIX.md](CROSS_VENUE_MATRIX.md).
