...
- Run `scripts/capture_api_responses.py` (or a per-API script) with small queries; write output to `unified_api_contracts/unified_api_contracts_external/<venue>/examples/`.
...
- In repos that use cassettes (UMI, UOI, market-tick-data-service), set `cassette_library_dir` to the shared unified-api-contracts mocks path (e.g. `unified-api-contracts/<api>/mocks/`).
