...
# --- VENUE_TO_DATA_SOURCE (migrated from unified-trading-services) ---
...
SYMBOLOGY_BY_VENUE: dict[str, dict[str, str]] = {
    ...
    "DERIBIT": {"format": "BASE-PERPETUAL|BASE-EXPIRY-STRIKE-C/P", "separator": "-", "example": "BTC-PERPETUAL"},
    "COINBASE-SPOT": {"format": "BASE-QUOTE", "separator": "-", "example": "BTC-USD"},
    "CME": {"format": "ROOT+MONTH+YEAR", "separator": "", "example": "XBT", "futures_example": "XBTZ4"},
...
CONTRACT_SPECS_BY_VENUE: dict[str, ContractSpec] = {
    ...
    "OKX": {"tick_size": 0.01, "lot_size": 0.001, "min_qty": 0.001, "max_qty": 1000.0, "expiry_format": "YYYY-MM-DD"},
    "BYBIT": {"tick_size": 0.01, "lot_size": 0.001, "min_qty": 0.001, "max_qty": 100.0},
    "DERIBIT": {"tick_size": 0.01, "lot_size": 0.1, "min_qty": 0.1, "max_qty": 10000.0, "expiry_format": "DDMMMYY"},
...
def get_venues_for_data_source(data_source: str) -> list[str]:
    """Get venues served by a data source."""
    return DATA_SOURCE_TO_VENUES.get(data_source, [])


def get_canonical_venue_for_dataset(dataset_id: str) -> str | None:
...
class DataSourceMapping:
    ...
    Migrated from unified-trading-services. All consumers import from api-contracts.
    """
