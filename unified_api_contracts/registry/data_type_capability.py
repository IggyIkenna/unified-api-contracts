...
DATA_TYPE_CAPABILITY_REGISTRY: Final[tuple[DataTypeCapability, ...]] = (
    ...
    *(
    ...
    # ------------------------------------------------------------------
    # Lowercase canonical forms (P1 migration 2026-08-08).
    # Uppercase entries above kept for backward compat with old manifest rows.
    # ------------------------------------------------------------------
    DataTypeCapability(
        asset_group=AssetGroup.SPORTS,
        ...
        batch_capable=True,
        requires_credentials=True,
        notes="Lowercase canonical form of FIXTURES_SCHEDULE (P1 migration)",
    ),
    DataTypeCapability(
        asset_group=AssetGroup.SPORTS,
        data_type="fixture_events",
