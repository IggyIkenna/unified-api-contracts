...
def _canonical_pipeline_mode_prefixes(asset_group: str) -> list[str]:
    ...
    for source in sorted(sources):
        ...
        if source in legacy:
            ...
            # written by the batch backfill, the live writer, or a replay re-fetch
            # (live=batch spine; CF-12 batch=live symmetry says the CELL has data either
            # way). Enumerating ONLY ``batch_``/``live_`` false-phantomed every
            # LIVE-captured cell whose object lives under ``pipeline_mode=live_<source>/``
            # (the 2026-07-11 prediction CF-15 finding: 13,292 KALSHI/POLYMARKET
            # ``book_snapshot_5``/``trades`` rows) — the same class of gap exists for
            # ``pipeline_mode=replay_<source>/`` objects (e.g. a databento/onchain_rpc
            # historical re-fetch). Adding a probe prefix can only REDUCE false-demotion
            # (never introduce one), so this is safe cross-AG.
