...
#   distinct writer bug.
# - "exchange_odds" / "fixed_odds": RETIRED split (2026-08-08 ruling 9 in
#   sports_taxonomy_p1_capture_and_contracts plan) — the distinction was
#   stamped per-instrument but is derivable from the VENUE (SportsVenueType
#   already encodes exchange vs sportsbook). New MTDS writes use "odds"
#   (InstrumentType.ODDS); these values remain ACCEPTED here because historical
#   manifest rows still carry them pending the P2 data migration.
#   CONTRACT_REGISTRY entries for ("sports","exchange_odds"/"fixed_odds","trades")
#   have been removed; lookup_contract has a backward-compat fallback.
# - "odds" (lowercase, generic): the canonical instrument_type for all sports
#   odds rows going forward, and for pre-fork rows the 2026-07-27 migration
#   didn't cover.
...
_INSTRUMENT_TYPE_ALIASES: dict[str, str] = {
    ...
    "fixture": "fixture",
    "league": "league",
    # exchange_odds / fixed_odds removed 2026-08-08 (sports_taxonomy_p1 ruling 9:
    # instrument_type split retired; derive from venue at read time).
    "prop": "prop",
...
VALID_DATA_TYPES_BY_AG_AND_INSTRUMENT_TYPE: dict[tuple[str, str], frozenset[str]] = {
    ...
    # (sports_taxonomy_p1 ruling 9: split retired; historical manifest rows still
    # carry these values but new writes stamp instrument_type=odds via InstrumentType.ODDS).
