...
VENUE_TO_ADAPTER_KEY: dict[str, str] = {
    ...
    # market_tick_data_service.adapters._umi_fred dispatch route, 2026-07-29). The
    # reference-data (instrument-catalogue) side is now also wired: 2026-08-03,
    # instruments-service/reference_data/adapters/tradfi/fred.py mirrors the FX
    # small-static-list precedent, emitting a same-shape duplicate of
    # FredAdapter.KEY_SERIES as instrument-catalogue rows (no vendor call, no
    # cross-service import — instruments-service cannot depend on MTDS).
    "FRED": "fred",
