"""
GCP Cloud SDK Pydantic request/response schemas.

Sync vs Async per client:
- Compute Engine: InstancesClient (sync) / InstancesAsyncClient (async)
- Cloud Run: ServicesClient, RevisionsClient (sync) / *AsyncClient (async)
- GCS: Client (sync only; wrap in asyncio.to_thread for async)
- BigQuery: Client (sync only; wrap in asyncio.to_thread for async)
- Pub/Sub, Cloud Build, Logging, Secret Manager, Artifact Registry, IAM
"""
