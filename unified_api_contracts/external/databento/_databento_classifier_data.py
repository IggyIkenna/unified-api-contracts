"""Declarative regex/lookup tables behind the Databento classifier facade.

Data split out of :mod:`unified_api_contracts.external.databento.databento_classifier`
(2026-07-12 >900-line cap) so the facade keeps the classification logic and this
module carries the pure symbol-shape constants: CME month codes, continuous-future
root sets, and every compiled regex used to recognize a Databento raw-symbol shape
(dated futures, FX futures, VX futures, combos, short/event/OSI options, ICE
futures, continuous-contract prefixes, CBOE user-defined strategies).

Import surface is UNCHANGED for consumers: every name here is private
(underscore-prefixed) and re-imported by ``databento_classifier`` only —
these are not part of the module's public ``__all__``.
"""
...
# put, strike=74000. Distinct from CME_SHORT_OPTION_RE because event contracts
