...
def normalize_binance_ticker(
    ...
    ts = ts_ms_to_datetime(raw.time or raw.closeTime)
...
def normalize_binance_orderbook(
    ...
    bids = to_levels([[p, q] for p, q in raw.bids])
    asks = to_levels([[p, q] for p, q in raw.asks])
...
def normalize_binance_order(raw: BinanceOrder, venue: str = "binance") -> CanonicalOrder:
    ts = ts_ms_to_datetime(raw.time or raw.updateTime)
...
def normalize_binance_derivative_ticker(
    ...
    if isinstance(raw, BinancePremiumIndex):
        symbol = raw.symbol or ""
        mark_price = to_decimal(raw.markPrice)
        index_price = to_decimal(raw.indexPrice)
        funding_rate = to_decimal(raw.lastFundingRate)
...
def normalize_binance_kline(raw: BinanceKline, symbol: str, venue: str = "binance") -> CanonicalOhlcvBar:
    ...
    return CanonicalOhlcvBar(
        ...
        venue=venue,
        symbol=symbol,
        open=d(raw.open_price),
        high=d(raw.high_price),
        low=d(raw.low_price),
        close=d(raw.close_price),
        volume=d(raw.volume),
        quote_volume=d(raw.quote_asset_volume),
