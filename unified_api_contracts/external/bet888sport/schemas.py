...
class 888sportOutcome(BaseModel):
...
class 888sportMarket(BaseModel):
    """Market (bet type) on a 888sport fixture."""

    market_type: str | None = Field(None, description="Market type (e.g. '1X2', 'Asian Handicap')")
    outcomes: list[888sportOutcome] = Field(default_factory=list)


class 888sportFixtureOdds(BaseModel):
    ...
    markets: list[888sportMarket] = Field(default_factory=list)
