...
def get_source_for_data_type(data_type: str) -> str | None:
    ...
    Case-insensitive: tries the input as-is first, then lowercased — old
    manifest rows carry uppercase tokens; canonical keys are lowercase post-P1.
    """
    return SPORTS_DATA_TYPE_TO_SOURCE.get(data_type) or SPORTS_DATA_TYPE_TO_SOURCE.get(data_type.lower())
...
def is_pre_launch_date(data_type: str, iso_date: str) -> bool:
    ...
    """
    source = SPORTS_DATA_TYPE_TO_SOURCE.get(data_type) or SPORTS_DATA_TYPE_TO_SOURCE.get(data_type.lower())
    if source is None:
        return False
    coverage_start = get_source_coverage_start(source, data_type.lower())
