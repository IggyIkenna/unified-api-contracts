...
class SchemaDefinition:
    ...
    def get_applicable_columns(self, dimensions: dict[str, str]) -> list[ColumnSchema]:
        ...
        return [col for col in self.columns if col.applies_to is None or bool(col.applies_to & dim_values)]
    ...
    def get_required_columns(self, dimensions: dict[str, str]) -> list[str]:
        ...
        return [
            col.name for col in self.get_applicable_columns(dimensions) if not self.is_nullable(col.name, dimensions)
        ]

    def get_nullable_columns(self, dimensions: dict[str, str]) -> list[str]:
        """Get list of nullable column names for given dimensions."""
        return [col.name for col in self.get_applicable_columns(dimensions) if self.is_nullable(col.name, dimensions)]
    ...
    def from_dict(cls, data: _RawSchema) -> SchemaDefinition:
        ...
        columns = [
            ColumnSchema(
                ...
                applies_to={str(x) for x in col.get("applies_to") or []} if col.get("applies_to") else None,
            )
