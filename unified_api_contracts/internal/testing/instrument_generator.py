...
class InstrumentGenerator:
    ...
    def generate_options_chain(
        ...
        _interval = strike_interval if strike_interval is not None else int(cfg["strike_interval_usd"])
        _range = strike_range_pct if strike_range_pct is not None else float(cfg["strike_range_pct"])
        ...
        _quarterly = quarterly_count if quarterly_count is not None else int(cfg["quarterly_expiries"])
    ...
    def generate_tradfi(self, ref_date: date) -> list[CanonicalInstrument]:
        ...
        for spec in TRADFI_FUTURES_SPECS:
            ...
            for exp_date in expiries:
                ...
                symbol = f"{root}{mc}{yy}"
                key = f"{venue}:{InstrumentType.FUTURE}:{symbol}"
                expiry_dt = datetime(exp_date.year, exp_date.month, exp_date.day, 20, 0, 0, tzinfo=UTC)
                instruments.append(
                    CanonicalInstrument(
                        instrument_key=key,
                        venue=venue,
