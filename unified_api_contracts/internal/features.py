...
class FeatureSnapshotRequest(BaseModel):
    ...
    instrument_id: str
    timestamp: datetime
    swing_lookback_window: int = Field(description="Horizon selector into swing_outcome_N columns (NOT forward shift)")
...
class CrossTimeframeFeatures(BaseModel):
    ...
    timeframe: str | None = Field(default=None, description="Primary timeframe of the trigger, e.g. '1h'")
...
class CrossInstrumentFeatures(BaseModel):
    ...
    instrument_ids: list[str] = Field(default_factory=list, description="All instruments in the feature set")
...
class LiquidationBandEntry(BaseModel):
    ...
    maintenance_margin_rate: float = Field(ge=0.0, description="Venue maintenance margin rate for this tier")
...
class LiquidationBandPredictionSnapshot(BaseModel):
    ...
    total_oi_usd: Decimal | None = Field(default=None, description="Total open interest in USD at prediction time")
