...
# ---------------------------------------------------------------------------
# Sports derived candles — odds_locf, odds_horizon_bucket, arbitrage_opportunity
#                          (1m / 15m / 1h)
#
# sports_taxonomy P1 (2026-08-08): odds_movement and odds_snapshot collapsed.
#   - odds_movement → now "odds" adapter → writes odds_ohlcv_{tf} (base loop above)
#   - odds_snapshot → now "odds_locf" adapter → writes odds_locf_{tf} (this loop)
#
...
# base granularity is 15m (UAC BASE_GRANULARITY_BY_DATA_TYPE) so the MDPS
# timeframe loop skips 1m at runtime — registering 1m here is harmless.
# ---------------------------------------------------------------------------

for _tf in _TIMEFRAMES_SPORTS:
    for _sports_derived_dt in ("odds_locf", "odds_horizon_bucket", "arbitrage_opportunity"):
        _register(
            _build(
                "sports",
                "odds",
