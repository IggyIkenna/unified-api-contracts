...
# Sports EXCHANGE_ODDS/FIXED_ODDS fork instrument_types -- fork RETIRED
# 2026-08-08 (sports_taxonomy_p1_capture_and_contracts_2026_08_08.md, operator
# ruling 9): exchange-vs-sportsbook is a property of the VENUE, not stamped
# per-instrument, so NEITHER instrument_type has its own CONTRACT_REGISTRY
# entry any more (the 2026-07-25 fork's ("sports", "exchange_odds"/
# "fixed_odds", "trades") entries were removed). Every lookup for one of these
# instrument_types -- "trades" included -- now falls back to
# ``CONTRACT_REGISTRY[(asset_group, "odds", data_type)]`` unconditionally: the
# row schema was always identical, only the partition key differed. This keeps
# existing GCS objects still partitioned under instrument_type=exchange_odds/
# fixed_odds (P2 migration re-stamps them) resolvable without a registry
# entry. Callers wanting the exchange-vs-sportsbook classification itself
# (not schema resolution) should derive it at read time via
...
def lookup_contract(
    *,
    ...
    Resolution order (each step falls through to the next on miss):
        ...
        5. Dual-read for the EXCHANGE_ODDS/FIXED_ODDS fork (fork RETIRED
           2026-08-08, ``sports_taxonomy_p1_capture_and_contracts_2026_08_08.md``
           operator ruling 9): for ``asset_group == "sports"`` and
           ``instrument_type`` in :data:`_SPORTS_ODDS_FORK_INSTRUMENT_TYPES`
           (``exchange_odds`` / ``fixed_odds``), fall back to
           ``CONTRACT_REGISTRY[(asset_group, "odds", data_type)]`` for EVERY
           data_type, including ``trades`` — neither instrument_type has its
           own registry entry any more, since exchange-vs-sportsbook is now
           derived at read time from the venue
           (``unified_api_contracts.registry.derive_sports_instrument_type``)
           rather than stamped per-instrument.
