...
# RETIRED (sports_taxonomy_p1_capture_and_contracts_2026_08_08.md): the
# exchange_odds/fixed_odds instrument_type split is being phased out —
# exchange-vs-sportsbook is a VENUE property (UAC SportsVenueType already
# encodes it). CONTRACT_REGISTRY entries for ("sports","exchange_odds"/"fixed_odds",
# "trades") have been removed. This tuple and the step-5 fallback below are kept
# for the migration window: existing manifest rows still carry these tokens until
# the P2 GCS + manifest restamp completes. Once P2 lands, remove this and step 5.
...
def lookup_contract(
    *,
    ...
    Resolution order (each step falls through to the next on miss):
        ...
        5. RETIRED migration-window fallback for ``exchange_odds`` / ``fixed_odds``
           (``sports_taxonomy_p1_capture_and_contracts_2026_08_08.md``): the
           dedicated CONTRACT_REGISTRY entries for those instrument_types have
           been removed; all sports-odds lookups now resolve under the unified
           ``"odds"`` instrument_type. For ``asset_group == "sports"`` and
           ``instrument_type`` in :data:`_SPORTS_ODDS_FORK_INSTRUMENT_TYPES`,
           fall back to ``CONTRACT_REGISTRY[(asset_group, "odds", data_type)]``.
           Remove this step once the P2 GCS + manifest restamp removes all
           ``instrument_type=exchange_odds``/``fixed_odds`` rows.
