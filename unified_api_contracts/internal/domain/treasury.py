...
# ---------------------------------------------------------------------------
# Treasury NAV computation helpers — canonical per Phase 3.D
# Pure functions; only depend on types defined above. No UTL/external deps.
# Canonical home: UAC (not strategy-service) since only UAC types are used.
# ---------------------------------------------------------------------------


def _make_stub_balance(source: TreasurySource, canned_usd: Decimal) -> TreasurySourceBalance:
    """Return a stub balance for sources whose SDK is not yet wired (pre-June-1)."""
    return TreasurySourceBalance(
        source=source,
        balance_usd=canned_usd,
        is_reachable=False,
        is_stub=True,
        error_message=(
            "STUB — source SDK not yet wired (Copper/CEFFU deferred to 2026-06-01). "
            "BLOCK_CRITICAL policy: do not use for live-trading decisions."
        ),
        as_of_timestamp=None,
    )


def compute_unified_nav(
    venue_margin_usd: Decimal | None = None,
    on_chain_usd: Decimal | None = None,
    copper_usd: Decimal | None = None,
    ceffu_usd: Decimal | None = None,
) -> TreasuryRollupResponse:
    """Compute unified-NAV across all four source axes.

    Per ``api_keys_wallets_accounts_readiness_2026_05_10.md`` Phase 3.D.
    Copper/CEFFU return stubs when None (SDK deferred to 2026-06-01).
    """
    now = datetime.now(UTC)

    copper_entry = (
        TreasurySourceBalance(
            source=TreasurySource.COPPER, balance_usd=copper_usd, is_reachable=True, is_stub=False, as_of_timestamp=now
        )
        if copper_usd is not None
        else _make_stub_balance(TreasurySource.COPPER, Decimal("0"))
    )
    ceffu_entry = (
        TreasurySourceBalance(
            source=TreasurySource.CEFFU, balance_usd=ceffu_usd, is_reachable=True, is_stub=False, as_of_timestamp=now
        )
        if ceffu_usd is not None
        else _make_stub_balance(TreasurySource.CEFFU, Decimal("0"))
    )
    venue_entry = TreasurySourceBalance(
        source=TreasurySource.SUB_ACCOUNT_HYPERLIQUID,
        balance_usd=venue_margin_usd if venue_margin_usd is not None else Decimal("0"),
        is_reachable=True,
        is_stub=False,
        as_of_timestamp=now,
    )
    onchain_entry = TreasurySourceBalance(
        source=TreasurySource.DEFI_HOT_WALLET,
        balance_usd=on_chain_usd if on_chain_usd is not None else Decimal("0"),
        is_reachable=True,
        is_stub=False,
        as_of_timestamp=now,
    )

    per_source = [copper_entry, ceffu_entry, venue_entry, onchain_entry]
    has_stubs = any(s.is_stub for s in per_source)
    reachable_count = sum(1 for s in per_source if s.is_reachable)
    total_nav = sum((s.balance_usd for s in per_source), Decimal("0"))

    if has_stubs:
        reconciliation_status = "stub"
    elif reachable_count < len(per_source):
    ...
    return TreasuryRollupResponse(
        ...
        has_stubs=has_stubs,
        source_count=reachable_count,
    )


def compute_nav_by_client(
    client_id: str,
    rollup: TreasuryRollupResponse,
    attribution_pct: Decimal | None = None,
    is_demo_single_client: bool = True,
) -> TreasuryNAVByClient:
    """Compute per-client NAV from the canonical rollup.

    Per ``wallet_treasury_client_flow_2026_05_10.md`` Phase 6.A.
    For the May-23 MVP, attribution_pct defaults to 100% (single demo client).
    """
    pct = attribution_pct if attribution_pct is not None else Decimal("100")
    scale = pct / Decimal("100")
    scaled_sources = [
        TreasurySourceBalance(
            source=s.source,
            balance_usd=(s.balance_usd * scale).quantize(Decimal("0.01")),
            is_reachable=s.is_reachable,
            is_stub=s.is_stub,
            as_of_timestamp=s.as_of_timestamp,
            error_message=s.error_message,
        )
        for s in rollup.per_source
    ]
    nav_usd = (rollup.total_nav_usd * scale).quantize(Decimal("0.01"))
    return TreasuryNAVByClient(
        client_id=client_id,
        nav_usd=nav_usd,
        attribution_pct=pct,
        per_source=scaled_sources,
        asof_timestamp=rollup.asof_timestamp,
        is_demo_single_client=is_demo_single_client,
    )
