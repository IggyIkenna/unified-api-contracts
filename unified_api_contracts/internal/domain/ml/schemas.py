...
HyperparameterConfigUnion = Annotated[
    LightGBMHyperparams
    | XGBoostHyperparams
    | CatBoostHyperparams
    | HuberHyperparams
...
class EnsembleMember(BaseModel):
    ...
    weight: float = Field(ge=0.0, le=1.0, description="Weight in [0, 1]. All weights should sum to 1.")
    hyperparameters: HyperparameterConfigUnion
...
class EnsembleConfig(BaseModel):
    ...
    meta_model_hyperparameters: HyperparameterConfigUnion | None = None
...
class TrainingPipelineConfig(BaseModel):
    ...
    multi_model: bool = Field(
        default=False,
        description=("False = 3-phase (stop at BASE_RESULTS). True = 5-phase (include META_LEARNING + META_RESULTS)."),
    )
    ...
    base_hyperparameters: HyperparameterConfigUnion = Field(
    ...
    meta_model_hyperparameters: HyperparameterConfigUnion = Field(
    ...
    pipeline_dependencies: list[str] | None = Field(
        default=None,
        description=("Pipeline IDs whose outputs become features for this pipeline's cross-pipeline phase"),
    )

    # --- Sports-specific ---
    pool_horizons: bool = Field(
        default=True,
        description=("Sports: train one base model across all time horizons with minutes_to_kickoff as feature"),
    )
    time_horizons: list[str] = Field(
        default_factory=list,
        description=("Sports fixture-relative horizons: ['T-24h', 'T-12h', 'T-6h', 'T-3h', 'T-15m', 'T-0', 'HT']"),
    )
    ...
    lineup_known_cutoff_minutes: int = Field(
        default=45,
        description=("Sports: conservative cutoff for when lineup is considered known (minutes before kickoff)"),
    )
    ...
    evaluation_metrics: list[str] = Field(
        ...
        description=(
            "Metrics to compute. Sports: ['poisson_nll', 'rps', 'brier']. Financial: ['accuracy', 'f1', 'auc']"
        ),
    )
...
class TargetTypeParams(BaseModel, frozen=True):
    """Per-target-type parameter bag for grid/config resolution.

    Known param sets by target_type:
    - swing_high / swing_low: swing_lookback_window (int), std_dev_threshold (float),
      breakout_threshold (float)
    - clv: odds_time_bucket (str), closing_line_window_minutes (int),
      min_odds_bookmakers (int)
    - xg: regression_target (str), max_goals_for_probs (int),
      include_opponent_strength (bool)
    - ht_delta: ht_state_features (list via comma-sep str), prediction_horizon (str)
...
class ModelVariantConfig(BaseModel):
    ...
    Target-type-specific parameters live in ``target_params`` — a dict whose
    valid keys depend on ``target_type``:
    ...
    def get_target_param(self, key: str, default: int | float | str | bool = 0) -> int | float | str | bool:
...
# ---------------------------------------------------------------------------
# Fixed vs Grid config — two-tier backtest architecture
# ---------------------------------------------------------------------------


class BacktestFixedConfig(BaseModel):
    """Tier 1: Fixed selections that determine which parameter space applies.

    These are picked once per experiment — they are NOT gridded. They act as
    lookup keys into the per-type parameter bags in GridDimensions.
    """

    instrument_id: str = Field(description="e.g. BTC-USDT, SPORTS:FOOTBALL:39")
    timeframe: str = Field(description="e.g. 5m, 1h, 4h, seasonal")
    target_type: TargetType
    model_type: ModelType = ModelType.LIGHTGBM
    pipeline_depth: int = Field(
        default=3,
        description="Phases to run: 3 = base only, 5 = base + meta, 6 = + cross-pipeline",
    )
    cv_strategy: str = Field(
        default="date",
        description="Walk-forward split: 'date' (financial) or 'seasonal' (sports)",
    )
    strategy_mode: str = Field(
        default="momentum",
        description="Strategy mode: momentum, mean_reversion, value_betting, stat_arb, etc.",
    )
    initial_capital: float = 100000.0


class GridDimensions(BaseModel):
    """Tier 2: Per-type parameter bags for combinatoric search.

    Each dict maps param_name → list of values to grid over.
    Only params relevant to the fixed selections are used.
    """

    target_type_params: dict[str, list[int | float | str | bool]] = Field(
        default_factory=dict,
        description=(
            "Grid params for the selected target_type. "
            "e.g. {'swing_lookback_window': [5, 10], 'std_dev_threshold': [1.5, 2.0]}"
        ),
    )
    strategy_mode_params: dict[str, list[int | float | str | bool]] = Field(
        default_factory=dict,
        description=(
            "Grid params for the selected strategy_mode. "
            "e.g. {'prediction_threshold': [0.55, 0.6], 'stop_loss_pct': [0.02, 0.03]}"
        ),
    )
    execution_algo_params: dict[str, dict[str, list[int | float | str | bool]]] = Field(
        default_factory=dict,
        description=(
            "Grid params per instruction_type.algorithm. "
            "e.g. {'TRADE': {'ALMGREN_CHRISS': {'horizon_secs': [300, 600]}}}"
        ),
    )


class BacktestExperimentConfig(BaseModel):
    """Complete backtest experiment: fixed selections + grid dimensions.

    Produces N shards where N = product of all grid dimension list lengths.
    Each shard is independently executable.
    """
