...
class RestartDetectedDetails(BaseModel):
    ...
    last_known_state: str | None = Field(default=None, description="last lifecycle state before unclean shutdown")
...
class UpstreamNotReadyDetails(BaseModel):
    """Details for UPSTREAM_NOT_READY event — emitted when a service's upstream data is missing."""
...
class ShardIncompleteDetails(BaseModel):
    """Details for SHARD_INCOMPLETE event — emitted when expected venues are missing after write."""
