...
class HalfTimeFeatureRecord(BaseModel):
    ...
    home_ht_goals_avg: float | None = Field(default=None, description="Home team historical avg first-half goals")
    home_ht_win_rate: float | None = Field(default=None, description="Home team historical HT win rate [0.0, 1.0]")
    ...
    pred_2h_home_goals: float | None = Field(default=None, description="Predicted second-half home goals")
    pred_2h_away_goals: float | None = Field(default=None, description="Predicted second-half away goals")
    ...
    ht_performance_shots_home: float | None = Field(default=None, description="Home shots in first half")
    ht_performance_shots_away: float | None = Field(default=None, description="Away shots in first half")
    ...
    ht_performance_xg_home: float | None = Field(default=None, description="Home expected goals in first half")
    ht_performance_xg_away: float | None = Field(default=None, description="Away expected goals in first half")

    # HT delta features
    ht_delta_goals: float | None = Field(default=None, description="Goal delta (home - away) at half time")
    ht_delta_possession: float | None = Field(default=None, description="Possession delta (home - away) at half time")
    ht_delta_xg: float | None = Field(default=None, description="xG delta (home - away) at half time")

    # HT momentum
    ht_momentum_home: float | None = Field(
        default=None, description="Home momentum score (goals + shots + xG weighted)"
    )
    ht_momentum_away: float | None = Field(default=None, description="Away momentum score (negative of home momentum)")
...
class SeasonContextFeatureRecord(BaseModel):
    ...
    round_name: str | None = Field(default=None, description="Round name (e.g. 'Regular Season - 10')")
...
class VenueContextFeatureRecord(BaseModel):
    ...
    altitude_m: float | None = Field(default=None, description="Stadium altitude above sea level in metres")
    stadium_capacity: int | None = Field(default=None, description="Stadium capacity (proxy for crowd pressure)")
    surface_type: str | None = Field(default=None, description="Pitch surface: grass | artificial | hybrid")
    is_neutral_venue: bool | None = Field(default=None, description="True if match is played at a neutral venue")
...
class SportsMLPredictionRecord(BaseModel):
    ...
    pred_home_win: float | None = Field(default=None, description="Predicted home win probability [0.0, 1.0]")
    pred_draw: float | None = Field(default=None, description="Predicted draw probability [0.0, 1.0]")
    pred_away_win: float | None = Field(default=None, description="Predicted away win probability [0.0, 1.0]")

    # Goals predictions
    pred_home_goals: float | None = Field(default=None, description="Predicted home goals (expected value)")
    pred_away_goals: float | None = Field(default=None, description="Predicted away goals (expected value)")
