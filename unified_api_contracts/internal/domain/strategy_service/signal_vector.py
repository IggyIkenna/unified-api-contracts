...
class SignalVectorRecord(BaseModel):
    ...
    sizing_confidence: float = Field(ge=0.0, le=1.0, description="Regime certainty * data quality score.")
    ...
    source_features_used: list[str] = Field(
        ...
        description=("Base feature names consumed during assembly. Used to enforce meta-model orthogonality."),
    )

    # Sub-component breakdowns (for explainability and debugging)
    direction_subcomponents: dict[str, float] = Field(
        default_factory=dict,
        description=("Named components contributing to direction_signal (e.g. cascade_confidence_score, cot_net)."),
    )
    vol_subcomponents: dict[str, float] = Field(
        default_factory=dict,
        description=("Named components contributing to vol_signal (e.g. vrp_z_score_252d, skew_25d, kurtosis_20)."),
    )
...
class MetaSignalRecord(BaseModel):
    ...
    meta_signal: float = Field(ge=-1.0, le=1.0, description="Weighted combination of signal vector components.")
    meta_confidence: float = Field(ge=0.0, le=1.0, description="Model confidence in meta_signal direction.")

    # Learned weights over signal vector dimensions (from LR coefficients or SHAP)
    signal_weights: dict[str, float] = Field(
        description=("Weight per signal dimension: direction, vol, timing, sizing. Sums approx 1.0 after rescaling.")
    )
    ...
    is_fallback_equal_weight: bool = Field(
        default=False,
        description=(
            "True when no trained model loaded; equal-weight {direction:0.4, vol:0.3, timing:0.2, sizing:0.1}."
        ),
    )
    meta_model_version: str = Field(default="", description="Version tag of the deployed meta-model artifact.")
