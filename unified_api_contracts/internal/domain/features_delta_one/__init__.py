...
FEATURES_SCHEMA = SchemaDefinition(
    ...
    columns=[
        ...
        ColumnSchema(
            ...
            dtype="datetime64[ns]",
            nullable=False,
            description=("Timestamp when feature became available (timestamp + 500ms synthetic delay)"),
        ),
        ColumnSchema(
            name="instrument_id",
            dtype="string",
...
def validate_feature_columns_not_null(
    ...
    for col in feature_columns:
        ...
        if null_count > 0:
            ...
            errors.append(
                f"Feature column '{col}' has {null_count} NaN values{ctx} - calculated features should never be NaN"
            )
