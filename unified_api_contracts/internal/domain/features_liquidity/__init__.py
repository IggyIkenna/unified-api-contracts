...
class BookDepthFeature1m(BaseModel):
    ...
    bid_depth_10bps: float | None = Field(default=None, description="Cum bid depth within 10 bps / ADV")
    bid_depth_25bps: float | None = Field(default=None, description="Cum bid depth within 25 bps / ADV")
    bid_depth_50bps: float | None = Field(default=None, description="Cum bid depth within 50 bps / ADV")
    bid_depth_100bps: float | None = Field(default=None, description="Cum bid depth within 100 bps / ADV")
    bid_depth_200bps: float | None = Field(default=None, description="Cum bid depth within 200 bps / ADV")
    bid_depth_300bps: float | None = Field(default=None, description="Cum bid depth within 300 bps / ADV")
    bid_depth_500bps: float | None = Field(default=None, description="Cum bid depth within 500 bps / ADV")
    ...
    adv_30d_usd: float | None = Field(default=None, description="30-day rolling average daily volume (USD)")
...
class LiquidityWallEvent(BaseModel):
    ...
    add_pressure: float | None = Field(default=None, description="Recent size additions at this level")
    cancel_pressure: float | None = Field(default=None, description="Recent size cancellations at this level")
    schema_version: str = "1.0.0"
...
class LiquidationClusterFeature1m(BaseModel):
    ...
    long_short_cluster_asymmetry: float | None = Field(
        default=None,
        description=("(short_cluster_usd - long_cluster_usd) / (short_cluster_usd + long_cluster_usd)"),
    )
...
class FlowInteractionFeature1m(BaseModel):
    ...
    cvd_usd: float | None = Field(default=None, description="Taker buy USD - taker sell USD this minute")
    cvd_5m_usd: float | None = Field(default=None, description="Rolling 5-min CVD in USD")

    # Taker imbalance ratio: buy / (buy + sell)
    taker_buy_ratio: float | None = Field(default=None, description="Taker buy USD / total taker USD")
...
class CompositeSRFeature1m(BaseModel):
    ...
    top_bid_sr_price: Decimal | None = Field(default=None, description="Strongest composite bid SR price")
    top_bid_sr_score: float | None = Field(default=None, description="Composite score [0-1] for top bid SR")
    top_ask_sr_price: Decimal | None = Field(default=None, description="Strongest composite ask SR price")
    top_ask_sr_score: float | None = Field(default=None, description="Composite score [0-1] for top ask SR")
