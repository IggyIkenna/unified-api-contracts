...
class CommoditySignal(BaseModel):
    ...
    master_signal: float = Field(description="Composite signal in [-1, 1]; negative = bearish, positive = bullish")
    regime: RegimeState = Field(default=RegimeState.UNKNOWN, description="Current market regime classification")
    regime_confidence: float = Field(default=0.0, description="Confidence in regime classification [0, 1]")
    factors: list[FactorValue] = Field(default_factory=list, description="Individual factor contributions")
