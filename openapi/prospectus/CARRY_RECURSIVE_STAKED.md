...
|---|---|---|---|---|
| `loop_borrow` | `borrow` | true | `lending` | `aave_v3`, `kamino` |
| `loop_collateral` | `lend` | true | `lending` | `aave_v3`, `kamino` |
| `loop_stake` | `stake` | true | `staking` | `jito`, `lido`, `marinade`, `rocketpool` |

**Conditional leg constraints:**

| Leg | Constraint (kind / params / fallback) | Condition |
|---|---|---|
| `loop_borrow` | `leg_constraint:requires_atomic_bundle · bundle_id=recursive_loop` | All loop iterations execute atomically on-chain (ATOMIC_ON_CHAIN composite call). |
| `loop_collateral` | `leg_constraint:requires_atomic_bundle · bundle_id=recursive_loop` | All loop iterations execute atomically on-chain (ATOMIC_ON_CHAIN composite call). |
| `loop_stake` | `leg_constraint:requires_atomic_bundle · bundle_id=recursive_loop` | All loop iterations execute atomically on-chain (ATOMIC_ON_CHAIN composite call). |
