"""Unit tests for phase5 (bonds_fx, alt_data) normalizers.
...
class TestArkhamNormalizers:
    def test_normalize_arkham_token_flow(self):
        ...
        result = normalize_arkham_token_flow(raw)
        assert isinstance(result, CanonicalOnChainMetric)
        assert result.metric_type == "entity_flow"
    ...
    def test_normalize_arkham_net_flow(self):
        ...
        raw = ArkhamNetFlow(
            ...
            token_symbol="BTC",
            time_window="24h",
            inflow_usd=1000000.0,
            outflow_usd=500000.0,
            net_flow_usd=500000.0,
        )
        result = normalize_arkham_net_flow(raw)
        assert isinstance(result, CanonicalOnChainMetric)
        assert result.metric_type == "net_flow"
    ...
    def test_normalize_arkham_alert_event(self):
        ...
        raw = ArkhamAlertEvent(
            ...
            chain="ethereum",
            token_symbol="BTC",
            usd_value=10000000.0,
            from_entity="unknown_wallet",
            to_entity="Coinbase",
            is_exchange_inflow=True,
            is_exchange_outflow=False,
            alert_type="large_transfer",
        )
        result = normalize_arkham_alert_event(raw)
        assert isinstance(result, CanonicalOnChainMetric)
        assert result.metric_type == "large_transfer"
        assert result.venue == "arkham"
...
class TestPythNormalizers:
    def test_normalize_pyth_price_feed_valid(self):
        ...
        raw = PythPriceFeed(
            ...
            conf=5000,
            expo=-2,
            publish_time=1704067200000000,  # microseconds
        )
        result = normalize_pyth_price_feed(raw)
        assert isinstance(result, CanonicalOraclePriceFeed)
        assert result.price == Decimal("42000.00")
        assert result.venue == "pyth"
        assert result.timestamp.tzinfo is not None
    ...
    def test_normalize_pyth_price_feed_negative_expo(self):
        ...
        result = normalize_pyth_price_feed(raw)
        assert result is not None
        assert result.price == Decimal("1000") * Decimal("10") ** (-8)
