...
# ---------------------------------------------------------------------------
# lookup_contract — "odds" instrument_type resolves directly
# ---------------------------------------------------------------------------
...
def test_lookup_contract_unknown_instrument_type_raises() -> None:
    with pytest.raises(SchemaContractNotFoundError):
        lookup_contract(asset_group="sports", instrument_type="exchange_odds", data_type="trades")
...
def test_every_sports_odds_family_contract_registry_entry_is_matrix_reachable() -> None:
    ...
    registry (ml_training manifests, sports reference/derived/feature
    contracts, MDPS candle-feature families, ...) is outside what this
    market-data validity matrix models at all.
    """
    from unified_api_contracts.registry.market_data_categories import (
