...
# ---------------------------------------------------------------------------
# Retired exchange_odds / fixed_odds backward-compat fallback
#
# sports_taxonomy_p1_capture_and_contracts_2026_08_08.md: exchange_odds and
# fixed_odds are retired as first-class instrument_types. Their CONTRACT_REGISTRY
# entries are removed; lookup_contract() step-5 falls them back to the "odds"
# contract. Manifest rows carrying these tokens remain until P2 migration.
# ---------------------------------------------------------------------------


def test_legacy_odds_trades_still_registered() -> None:
    """("sports","odds","trades") is a P2 compat alias for existing GCS shards."""
    assert ("sports", "odds", "trades") in CONTRACT_REGISTRY
    assert CONTRACT_REGISTRY[("sports", "odds", "trades")] is SPORTS_ODDS_TRADES


def test_exchange_odds_not_in_contract_registry() -> None:
    """exchange_odds no longer has its own first-class CONTRACT_REGISTRY entry."""
    assert ("sports", "exchange_odds", "trades") not in CONTRACT_REGISTRY


def test_fixed_odds_not_in_contract_registry() -> None:
    """fixed_odds no longer has its own first-class CONTRACT_REGISTRY entry."""
    assert ("sports", "fixed_odds", "trades") not in CONTRACT_REGISTRY


def test_lookup_contract_exchange_odds_falls_back_to_odds_contract() -> None:
    """Retired exchange_odds falls back to the odds contract via step-5 compat."""
    contract = lookup_contract(asset_group="sports", instrument_type="exchange_odds", data_type="trades")
    assert contract is SPORTS_ODDS_TRADES


def test_lookup_contract_fixed_odds_falls_back_to_odds_contract() -> None:
    """Retired fixed_odds falls back to the odds contract via step-5 compat."""
    contract = lookup_contract(asset_group="sports", instrument_type="fixed_odds", data_type="trades")
    assert contract is SPORTS_ODDS_TRADES


# ---------------------------------------------------------------------------
# SPORTS_ODDS — canonical data_type=odds (trades→odds collapse, ruling 4)
# sports_taxonomy_p1_capture_and_contracts_2026_08_08.md
# ---------------------------------------------------------------------------


def test_sports_odds_registered_as_primary_key() -> None:
    assert ("sports", "odds", "odds") in CONTRACT_REGISTRY
    assert CONTRACT_REGISTRY[("sports", "odds", "odds")] is SPORTS_ODDS
...
def test_sports_odds_shares_raw_odds_columns_plus_in_play() -> None:
    """SPORTS_ODDS is the SPORTS_ODDS_TRADES schema plus the in_play axis."""
    legacy_names = {c.name for c in SPORTS_ODDS_TRADES.columns}
    new_names = {c.name for c in SPORTS_ODDS.columns}
    assert new_names == legacy_names | {"in_play"}
...
# ---------------------------------------------------------------------------
# lookup_contract backward-compat: retired exchange_odds/fixed_odds fall back
# to the "odds" contract for all data_types until P2 migrates manifest rows.
# (sports_taxonomy_p1_capture_and_contracts_2026_08_08.md)
# ---------------------------------------------------------------------------


def test_lookup_contract_odds_path_resolves_directly() -> None:
    """The canonical odds path resolves directly — unaffected by P1 changes."""
    contract = lookup_contract(asset_group="sports", instrument_type="odds", data_type="trades")
    assert contract is SPORTS_ODDS_TRADES


def test_lookup_contract_exchange_odds_falls_back_for_all_data_types() -> None:
    """exchange_odds falls back to the odds contract for every data_type."""
...
def test_lookup_contract_fixed_odds_falls_back_for_all_data_types() -> None:
    """fixed_odds falls back to the odds contract for every data_type."""
    contract = lookup_contract(asset_group="sports", instrument_type="fixed_odds", data_type="sports_odds_snapshot")
    assert contract is SPORTS_ODDS_SNAPSHOT


def test_lookup_contract_compat_fallback_is_sports_only() -> None:
...
def test_every_sports_odds_family_contract_registry_entry_is_matrix_reachable() -> None:
    ...
    Scoped to the odds-shape family the matrix declares
    (fixture/prop/odds) rather than the whole CONTRACT_REGISTRY -- most of the
    registry (ml_training manifests, sports reference/derived/feature contracts,
    MDPS candle-feature families, ...) is outside what this market-data validity
    matrix models at all. exchange_odds/fixed_odds were removed from the matrix
    in sports_taxonomy_p1_capture_and_contracts_2026_08_08.md.
    """
    from unified_api_contracts.registry.market_data_categories import (
