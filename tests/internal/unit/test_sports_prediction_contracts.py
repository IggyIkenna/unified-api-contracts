...
# to the canonical odds contract via _SPORTS_ODDS_FORK_INSTRUMENT_TYPES step 5
# (contracts.py). The explicit CONTRACT_REGISTRY entries were removed 2026-08-08
# (sports_taxonomy_p1_capture_and_contracts_2026_08_08.md) but the step 5
# fallback is retained during the P2 GCS migration window so any live manifest
# row carrying instrument_type=exchange_odds/fixed_odds still resolves.
# ---------------------------------------------------------------------------


def test_lookup_contract_legacy_odds_path_still_resolves_directly() -> None:
    """The canonical odds path is untouched."""
    contract = lookup_contract(asset_group="sports", instrument_type="odds", data_type="trades")
    assert contract is SPORTS_ODDS_TRADES


def test_lookup_contract_retired_fork_types_fall_back_to_legacy_odds_trades() -> None:
    """exchange_odds/fixed_odds have no own CONTRACT_REGISTRY entry — step 5
    fallback in lookup_contract returns the canonical odds contract for all
    data_types, including trades."""
...
def test_lookup_contract_dual_reads_unforked_odds_data_type_via_exchange_odds() -> None:
    """sports_odds_snapshot resolves for exchange_odds via step 5 fallback."""
