...
def test_legacy_odds_trades_still_registered() -> None:
    """The odds entry must remain — it is the unified forward-written type from P1."""
...
# ---------------------------------------------------------------------------
# lookup_contract backward-compat: retired exchange_odds/fixed_odds instrument_types
# (sports_taxonomy_p1_capture_and_contracts_2026_08_08.md, operator ruling 9).
# The dedicated CONTRACT_REGISTRY entries for these types were removed in P1.
# Existing GCS/manifest rows still carry these instrument_types until P2
# re-stamps them to "odds". The _SPORTS_ODDS_FORK_INSTRUMENT_TYPES fallback
# in lookup_contract() is the PERMANENT backward-compat mechanism for this.
# ---------------------------------------------------------------------------


def test_lookup_contract_legacy_odds_path_still_resolves_directly() -> None:
    """The legacy path is untouched by the backward-compat fallback."""
    contract = lookup_contract(asset_group="sports", instrument_type="odds", data_type="trades")
    assert contract is SPORTS_ODDS_TRADES


def test_lookup_contract_exchange_fixed_odds_fall_back_to_odds() -> None:
    """exchange_odds/fixed_odds instrument_types were retired in P1; their dedicated
    registry entries are gone. Lookups fall through to ("sports", "odds", data_type)
    via the _SPORTS_ODDS_FORK_INSTRUMENT_TYPES backward-compat step.
    """
...
def test_lookup_contract_backward_compat_via_exchange_odds() -> None:
    """sports_odds_snapshot resolves via the backward-compat fallback for exchange_odds."""
...
def test_lookup_contract_backward_compat_via_fixed_odds() -> None:
    contract = lookup_contract(asset_group="sports", instrument_type="fixed_odds", data_type="sports_odds_snapshot")
    assert contract is SPORTS_ODDS_SNAPSHOT


def test_lookup_contract_backward_compat_fallback_is_sports_only() -> None:
