...
# ---------------------------------------------------------------------------
# Legacy read fallback — exchange_odds / fixed_odds retired 2026-08-08
# (sports_taxonomy_p1_capture_and_contracts_2026_08_08.md). CONTRACT_REGISTRY
# no longer has entries keyed on these instrument_types; pre-P2 data carrying
# them routes through the step-5 fallback in lookup_contract to the "odds"
# contract. These tests confirm that fallback still resolves.
# ---------------------------------------------------------------------------


def test_legacy_exchange_odds_routes_to_odds_contract() -> None:
    contract = lookup_contract(asset_group="sports", instrument_type="exchange_odds", data_type="trades")
    assert contract is SPORTS_ODDS_TRADES


def test_legacy_fixed_odds_routes_to_odds_contract() -> None:
    contract = lookup_contract(asset_group="sports", instrument_type="fixed_odds", data_type="trades")
    assert contract is SPORTS_ODDS_TRADES


def test_exchange_fixed_odds_not_in_contract_registry() -> None:
...
# ---------------------------------------------------------------------------
# lookup_contract legacy fallback: pre-P2 exchange_odds / fixed_odds data
# (retired 2026-08-08; step-5 fallback in contracts.py routes these tokens
# to the "odds" contract during the P2 re-stamp window).
# ---------------------------------------------------------------------------


def test_lookup_contract_legacy_odds_path_still_resolves_directly() -> None:
    """The canonical "odds" path is unaffected by the retirement."""
    contract = lookup_contract(asset_group="sports", instrument_type="odds", data_type="trades")
    assert contract is SPORTS_ODDS_TRADES


def test_lookup_contract_legacy_exchange_odds_routes_to_odds_for_trades() -> None:
    """exchange_odds has no own CONTRACT_REGISTRY entry — step-5 fallback applies."""
...
def test_lookup_contract_legacy_fixed_odds_routes_to_odds_for_trades() -> None:
...
def test_lookup_contract_dual_reads_unforked_odds_data_type_via_exchange_odds() -> None:
    """Legacy exchange_odds still resolves for non-trades data_types via the fallback."""
...
def test_every_sports_odds_family_contract_registry_entry_is_matrix_reachable() -> None:
    ...
    (fixture/prop/odds) rather than the whole
